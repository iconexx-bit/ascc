"""Step 6: turn persisted Fact history into BridgeFact edges for this run.

See CLAUDE.md, "### Step 6: store consumer (cross-run identity)". A Fact
written by Step 5 carries a MatchKey's fields flat in its payload (payload_v
...
`_to_fact`, Step 5). A pre-Step-6 (v0) fact carries no such fields and needs
...
def facts_to_bridge_facts(
    ...
    """Historical facts filtered for Union-Find, per CLAUDE.md Step 6:
